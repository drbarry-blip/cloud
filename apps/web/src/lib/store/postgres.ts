...
/** Postgres store. Schema lives in apps/web/migrations. */
export class PostgresStore implements Store {
  constructor(private pool: pg.Pool) {}

  static fromUrl(url: string): PostgresStore {
    return new PostgresStore(new pg.Pool({ connectionString: url, max: 5 }));
  }
    ...
    const { rows } = await this.pool.query<LeadRow>(
      `INSERT INTO leads (email, sources, marketing_consent)
       ...
       ON CONFLICT (email) DO UPDATE SET
         sources = CASE WHEN $2 = ANY(leads.sources) THEN leads.sources ELSE array_append(leads.sources, $2) END,
  ...
  async confirmLead(id: string, now: Date) {
    const { rows } = await this.pool.query<LeadRow>(
      "UPDATE leads SET confirmed_at = COALESCE(confirmed_at, $2) WHERE id = $1 RETURNING *",
      [id, now],
    );
    return rows[0] ? toLead(rows[0]) : null;
  }
  ...
  async unsubscribeLead(id: string, now: Date) {
    await this.pool.query("UPDATE leads SET unsubscribed_at = COALESCE(unsubscribed_at, $2) WHERE id = $1", [id, now]);
  }
  ...
  async setNurture(id: string, step: number, nextAt: Date | null) {
    await this.pool.query("UPDATE leads SET nurture_step = $2, nurture_next_at = $3 WHERE id = $1", [id, step, nextAt]);
  }

  async recordReplyCheck(meta: ReplyCheckMeta) {
    await this.pool.query(
      "INSERT INTO reply_checks (verdict, categories, clinic_type, mode, lead_id) VALUES ($1, $2, $3, $4, $5)",
      [meta.verdict, meta.categories, meta.clinicType, meta.mode, meta.leadId],
    );
  }
